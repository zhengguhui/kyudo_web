...
function genLocationOne(shop, i){


    console.log(shop.length + " " + i);
    if (i == 88){
        console.log(JSON.stringify(shop));
        return ;
    }
    console.log(shop[i].address);
    if (shop[i].address == undefined){
        genLocationOne(shop, i + 1);
        return;
    }
	geocoder.geocode( { 'address': shop[i].address}, function(results, status) {
      if (status == google.maps.GeocoderStatus.OK) {
            console.log(shop.length + " " + i);
            sleep(1100);
            console.log(results[0].geometry.location.lat());
            shop[i].location = {"lat" : results[0].geometry.location.lat(), "lng": results[0].geometry.location.lng()}
            genLocationOne(shop, i + 1);
      } 
    });
